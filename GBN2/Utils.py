...
# 发送的数据帧
class PDU:
    def __init__(self, num_to_send, pdu_to_send, status, acked_num, data, checksum):
    ...
    def get_pdu(self):
        # 使用字典封装frame
        pdu = {'num_to_send': self.num_to_send,
...
class Config:
    def __init__(self):
        ...
        self.send_port = 9998                                 # 服务端的端口
        self.send_addr = (self.ip, self.send_port)          # 客户端地址
        self.recv_port = 9999                                   # 客户端端口
        self.recv_addr = (self.ip, self.recv_port)              # 服务端地址
        ...
        self.send_file = 'test2.pdf'  # 发送的文件
        self.send_log = self.send_file + '.log'     # 发送方日志
        self.recv_file = 'chpt3_recv.pdf'           # 接受后写入的文件
