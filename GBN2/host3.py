...
def receive_pdu(pdu, host2_addr):
    global host1_config
    global host1_socket
    global host1_send_file
    global host1_send_log
...
def receiver():
    ...
    while True:
        ...
        if pdu['is_ack'] == -1:  # 如果是数据帧
            print("!!!!!!!!")
            # 开启一个新线程处理收到的pdu
            threading.Thread(name='receive_pdu',
                             target=receive_pdu,
                             args=(pdu, host2_addr)).start()
            ...
            ack = pdu['data']
            # 开启一个新线程处理收到的ack
            threading.Thread(name='receive_ack',
                             target=receive_ack,
                             args=(ack, )).start()
        else:
            print("[ERROR] is_ack$$$$$$$$")
        # time.sleep(0.1)
    print("receive process ended")
...
def kill_threads():
    ...
    global receive_finished
    while True:
        if send_finished and receive_finished:
            os.kill(os.getpid(), signal.SIGTERM)
...
if __name__ == '__main__':
    ...
    # 阻塞模式
    host1_socket.setblocking(0)
    host1_socket.settimeout(10)
    ...
    if cmd == "0":
        ...
        kill_threads = threading.Thread(name='kill_threads', target=kill_threads)
